...
class ClosestColorsFrame:
    ...
    def __init__(
        ...
        tk.Label(title_frame, text="Closest Colors").pack(side=tk.LEFT, expand=True)
    ...
    def update_buttons(self):
        for color_var, button in zip(
            XKCDColor.get_nclosest_xkcd_colors(
                n=self.N_COLS * self.N_ROWS, color=self.chosen_color
            ),
            self.buttons,
        ):
